...
import com.footyandsweep.apisweepstakeengine.event.SweepstakeMessageListener;
...
public class KafkaConfig {
  ...
  public NewTopic sweepstakeEvents() {
    return TopicBuilder.name("api-sweepstake-events-topic").replicas(1).build();
  }
  ...
  @Bean
  public ConcurrentKafkaListenerContainerFactory<String, BaseEvent> SweepstakeEventKafkaListenerContainerFactory() {
    ConcurrentKafkaListenerContainerFactory<String, BaseEvent> factory = new ConcurrentKafkaListenerContainerFactory<>();
    factory.setConsumerFactory(sweepstakeEventConsumerFactory());

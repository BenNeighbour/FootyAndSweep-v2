...
public class GatewayFilter
    implements GatewayFilterFactory<com.footyandsweep.apigatewayservice.GatewayFilter.Config> {
  ...
  public org.springframework.cloud.gateway.filter.GatewayFilter apply(Config config) {
    return (exchange, chain) -> {
      ...
      if (!"ws".equals(scheme) && !"wss".equals(scheme)) return chain.filter(exchange);
        ...
        //                URI wsRequestUrl =
        // UriComponentsBuilder.fromUri(requestUrl).scheme(wsScheme).build().toUri();
